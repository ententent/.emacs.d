...
*** pdf-tools
**** Windows
- 借助 =M-x package-list-packages= 从 =melpa-stable= 安装 =pdf-tools=
- 使用 =scoop= 安装 =msys2=
- 打开 =msys2= 下的 =mingw64.exe= 执行
  - =pacman -S mingw-w64-x86_64-texlive-full=
  - =pacman -S mingw-w64-x86_64-emacs-pdf-tools-server=
- 将 =E:\Scoop\apps\msys2\current\mingw64\bin= 添加到环境变量
- 执行 =pdf-tools-install=
**** Arch Linux
- 执行 =yay -S emacs-pdf-tools-git=
#+BEGIN_SRC emacs-lisp
